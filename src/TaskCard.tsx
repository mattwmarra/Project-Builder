  ...
  return (
    <div>
      <Card
        ...
        text={
          setPriorityColor(state.props.priority) === "light" ? "dark" : "white"
        }
        ...
        <Accordion>
          ...
          <Accordion.Collapse eventKey="0">
            <Card.Body background="white">
              {!state.editContent ? (
                <Card.Text id={"content"} onDoubleClick={handleDoubleClick}>
                  {state.props.content}{" "}
                </Card.Text>
              ) : (
                <Form.Control
              ...
              <Card.Subtitle>
                Last Modified:{" "}
                {props.task.lastMovedDate !== undefined
                  ? convertToLocal(props.task.lastMovedDate)
                  : "N/A"}
              </Card.Subtitle>
              <br></br>
              {!state.editPriority ? (
                <Card.Subtitle
                  id={"priority"}
                  onDoubleClick={handleDoubleClick}
                  variant={setPriorityColor(state.props.priority)}
                >
                  Priority: {capitalize(state.props.priority)}
                </Card.Subtitle>
              ) : (
                <DropdownButton
                  title={"Priority: " + capitalize(state.props.priority)}
                  variant={setPriorityColor(state.props.priority)}
                  style={{ height: 40, margin: 0 }}
                >
                  <Dropdown.Item onClick={() => changeCardPriority("low")}>
                    Low
                  </Dropdown.Item>
                  <Dropdown.Item onClick={() => changeCardPriority("medium")}>
                    Medium
                  </Dropdown.Item>
                  <Dropdown.Item onClick={() => changeCardPriority("high")}>
                    High
                  </Dropdown.Item>
                </DropdownButton>
              )}
